...
struct rtGpuMemRegion* getGpuMemRegion(rtMemoryRegionType memType) {
  switch (memType) {
    case rtMemoryRegion_UPLOAD: return m_stagingAllocations;
    case rtMemoryRegion_LOCAL: return m_devLocalAllocations;
  }
  return nullptr;
}
rtGpuMemBlock* allocateMemBlock(std::vector<rtGpuMemBlock*>& memAllocs,
  ...
  rtGpuMemBlock* memBlock = new rtGpuMemBlock;
...
rtGpuMemBlock* rtGpuMemRegion::allocateMemBlock(heapRequirementsInfo_tgfx reqs) {
  rtGpuMemBlock* memBlock =
...
void initMemRegions() {
  ...
  m_stagingAllocations = new rtGpuMemRegion;
  ...
  m_devLocalAllocations = new rtGpuMemRegion;
